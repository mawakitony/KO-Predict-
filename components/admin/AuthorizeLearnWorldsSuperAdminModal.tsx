    ...
    startTransition(async () => {
      try {
        ...
        if (!json.found || !json.identity) {
          setError("Aucun compte LearnWorlds trouvé avec cette adresse.");
          return;
        }
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/40 p-4 sm:items-center">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-5 shadow-xl">
        ...
        <p className="mt-2 text-sm text-slate-600">
          Vérifie l’identité LearnWorlds. Aucune promotion de rôle KO à cette
          étape.
        </p>
        ...
        {identity && lookup ? (
          <div className="mt-4 space-y-3 rounded-xl border border-slate-100 bg-slate-50 px-3 py-3">
            ...
            <label className="flex items-start gap-2 text-sm text-slate-700">
              ...
              />
              <span>
                Je confirme ajouter cette autorisation d’éligibilité. Cela ne
                promeut personne en super_admin KO Predict™.
              </span>
            </label>
            <button
              type="button"
