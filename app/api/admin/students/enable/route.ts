...
import { enableStudentAccess } from "@/lib/admin/account-access";
...
/**
 * POST /api/admin/students/enable
 * Body: { studentId: string }
 */
export async function POST(request: Request) {
  const gate = await requirePermissionApi(
    canDisableStudents,
    "Réactivation réservée aux administrateurs.",
  );
  ...
  const result = await enableStudentAccess(studentId);
